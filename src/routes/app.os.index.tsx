...
export const Route = createFileRoute("/app/os/")({ component: Page, head: () => ({ meta: [{ title: "Ordens de Serviço" }] }) });
...
function Page() {
  ...
  async function load() {
    const [o, c, v, f] = await Promise.all([
      supabase.from("ordens_servico").select("*, clientes(nome), veiculos(placa,marca,modelo)").order("created_at", { ascending: false }),
      supabase.from("clientes").select("id,nome").order("nome"),
      supabase.from("veiculos").select("id,placa,marca,modelo,cliente_id"),
  ...
  return (
    <>
      <PageHeader title="Ordens de Serviço" subtitle={`${list.length} OS no total`}
        action={
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild><Button className="bg-gradient-primary text-primary-foreground shadow-glow"><Plus className="h-4 w-4 mr-2" />Abrir OS</Button></DialogTrigger>
            <DialogContent>
              <DialogHeader><DialogTitle className="font-display">Abrir nova OS</DialogTitle></DialogHeader>
              <form onSubmit={criar} className="space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div><Label>Previsão entrega</Label><Input name="data_prevista" type="date" /></div>
                </div>
                <div><Label>Problema relatado</Label><Textarea name="descricao_problema" placeholder="O que o cliente relatou?" /></div>
                <DialogFooter><Button type="submit" className="bg-gradient-primary text-primary-foreground">Abrir OS</Button></DialogFooter>
              </form>
            </DialogContent>
          </Dialog>
        } />
      ...
      <div className="flex gap-2 flex-wrap mb-4">
        {["todos", "aberta", "em_andamento", "aguardando_pecas", "concluida", "cancelada"].map(s => (
          <button key={s} onClick={() => setFiltro(s)}
            ...
            {s === "todos" ? "Todos" : STATUS_OS[s]?.label ?? s}
          </button>
        ))}
      </div>

      {filtered.length === 0 ? <EmptyState icon={ClipboardList} title="Nenhuma OS" hint="Abra a primeira ordem de serviço." /> : (
        <div className="bg-card border rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead className="bg-muted text-xs uppercase text-muted-foreground">
              <tr><th className="text-left px-4 py-3">OS</th><th className="text-left px-4 py-3">Cliente / Veículo</th><th className="text-left px-4 py-3">Status</th><th className="text-left px-4 py-3 hidden md:table-cell">Entrada</th><th className="text-right px-4 py-3">Total</th></tr>
            </thead>
            <tbody>
              {filtered.map(o => {
                const st = STATUS_OS[o.status] ?? { label: o.status, cls: "" };
                return (
                  <tr key={o.id} className="border-t border-border hover:bg-muted/30 cursor-pointer" onClick={() => window.location.assign(`/app/os/${o.id}`)}>
                    <td className="px-4 py-3 font-mono text-primary">#{o.numero}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium">{o.clientes?.nome}</div>
                      <div className="text-xs text-muted-foreground">{o.veiculos?.placa} • {o.veiculos?.marca} {o.veiculos?.modelo}</div>
                    </td>
                    <td className="px-4 py-3"><span className={`inline-block px-2 py-0.5 rounded border text-xs ${st.cls}`}>{st.label}</span></td>
                    <td className="px-4 py-3 hidden md:table-cell text-muted-foreground">{fmtDate(o.data_entrada)}</td>
